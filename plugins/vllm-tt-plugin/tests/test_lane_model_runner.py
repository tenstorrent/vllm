...
"""Host-only tests for the lane-DP model-runner adapter.

No device execution; the lane executor's index logic runs on plain tensors with
fake runner/model collaborators. Requires the ttnn-enabled environment because
importing the plugin modules pulls in ttnn.
"""
...
def test_lane_executor_rejects_request_specific_rope():
    runner = SimpleNamespace(request_specific_rope=True)
    executor = TTLaneStepExecutor(runner)
    ...
    with pytest.raises(NotImplementedError, match="request-specific RoPE"):
        executor.execute_model(output, grammar_output=None)


def test_extract_step_device_decode_returns_scheduled_rows_in_order():
    executor = TTLaneStepExecutor(SimpleNamespace())
    ...
    sampled, logprobs = executor.extract_step(
        tt_out, None, model_input, scheduled_rows=[4, 1], is_decode=True
    )
...
def test_extract_step_device_prefill_returns_front_packed_tokens():
    executor = TTLaneStepExecutor(SimpleNamespace())
    ...
    sampled, logprobs = executor.extract_step(
        tt_out, None, model_input, scheduled_rows=[4, 1], is_decode=False
    )
...
def test_extract_step_host_decode_samples_full_slot_then_picks_scheduled_rows():
    captured: dict = {}
    runner = SimpleNamespace(
        lane_batch=SimpleNamespace(
            max_num_reqs=5, build_merged_sampling_metadata=lambda rows: rows
        ),
        host_sampler=_capturing_host_sampler(captured),
    )
    executor = TTLaneStepExecutor(runner)
    # Full slot logits: row r's argmax is token r.
    ...
    sampled, logprobs = executor.extract_step(
        logits, None, model_input, scheduled_rows=[4, 1], is_decode=True
    )

    assert captured["logits"].shape == (5, 8)  # sampled the whole slot batch once
    assert sampled.tolist() == [[4], [1]]  # rows 4 and 1, in scheduled order
    assert logprobs is None


def test_extract_step_host_prefill_scatters_logits_to_stable_rows():
    captured: dict = {}
    runner = SimpleNamespace(
        lane_batch=SimpleNamespace(
            max_num_reqs=5, build_merged_sampling_metadata=lambda rows: rows
    ...
    prefill_logits = torch.full((2, 8), -10.0)
    prefill_logits[0, 3] = 5.0  # scheduled request 0 -> token 3
    prefill_logits[1, 6] = 5.0  # scheduled request 1 -> token 6
    model_input = SimpleNamespace(perform_device_sampling=False, grammar_bitmask=[None])

    sampled, _ = executor.extract_step(
        prefill_logits, None, model_input, scheduled_rows=[4, 1], is_decode=False
    )

    full = captured["logits"]
    assert full.shape == (5, 8)  # scattered onto the full slot grid before sampling
...
def test_async_lane_decode_uses_lane_executor_extraction():
    calls = []

    class FakeLaneExecutor:
        def extract_step(
            self, tt_out, tt_log_probs, model_input, scheduled_rows, is_decode
        ):
            calls.append((tt_out, tt_log_probs, model_input, scheduled_rows, is_decode))
            return torch.tensor([[7]], dtype=torch.int32), None

    runner = SimpleNamespace(lane_executor=FakeLaneExecutor())
    ...
    completed = controller.complete_lane_decode_step(
    ...
    assert len(calls) == 1
    tt_out, tt_log_probs, seen_model_input, scheduled_rows, is_decode = calls[0]
