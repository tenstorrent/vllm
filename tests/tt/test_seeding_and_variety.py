from tests.tt.utils import RequestConfig, run_concurrent_batch, assert_varied, assert_deterministic, assert_pairwise_varied
import pytest
import random
...
class TestSeedingAndVariety:
    ...
    def test_seeding(self, tt_server, tt_model_name, max_batch_size):
        ...
        for _ in range(2):
            for i in range(seeds):
                configs.append(RequestConfig(prompt=prompt, max_tokens=10, temperature=1.5, top_k=50, seed=i * 100))
        ...
        assert_deterministic(all_greedy, "Greedy requests should produce the same output across positions and runs.")
        ...
        for i in range(seeds):
            ...
            assert_deterministic(results_for_seed, "Seeded requests should produce the same output across positions and runs.")
            different_seeds.append(results_for_seed[0])
        expected_variety = seeds//3
        assert_varied(different_seeds, expected_variety, "Seeded requests should produce different outputs for different seeds.")

        # Check first tokens to make sure prefill is varied
        prefill_different_seeds = [x[:1] for x in different_seeds]
        assert_varied(prefill_different_seeds, expected_variety, "First token should be varied for different seeds.")



    def test_different_seeds_produce_different_outputs(self, tt_server, tt_model_name, max_batch_size):
        """
        ...
        assert_varied(results, max_batch_size // 3, "Different seeds should produce different outputs.")
    ...
    def test_same_seeds_reproduce_across_batches(
        ...
        for i in range(max_batch_size):
            assert_deterministic(results_per_seed[i], f"Seed should produce the same output in different batches.")
    ...
    def test_batch1_seed_reproducible(
        self, tt_server, tt_model_name, seed
    ):
        """
        ...
        assert_deterministic(results, "Batch with 1 user with seed should produce reproducible outputs.")
    ...
    def test_batch1_no_seed_varied(self, tt_server, tt_model_name):
        ...
        for _ in range(10):
            results.extend(run_concurrent_batch(tt_server, tt_model_name, configs))
        assert_varied(results, 2, "Batch with 1 user with no seed should produce varied outputs.")
    ...
    def test_temperature_varied_in_batch(
        ...
        assert_varied(prefill_results, 2, "With temperature=5, first tokens should vary in batch.")

    def test_temperature_varied_between_batches(
        ...
        # First token (prefill)
        prefill_results = [x[:1] for x in results]
        assert_varied(prefill_results, 2, "With temperature=5, first tokens should vary on re-runs.")
    ...
    def test_topk(
        ...
        greedy_config = [RequestConfig(prompt=prompt, max_tokens=10, temperature=0) for _ in range(num_greedy)]

        topk_configs = [
            RequestConfig(prompt=prompt, max_tokens=10, top_k=10, temperature=10)
        ...
        for results in [results_1, results_2]:
            ...
            assert_deterministic(greedy_results, "Greedy requests should produce the same output.")
            assert_varied(non_greedy_results, 2, "With top_k=10, outputs should vary in batch.")

            # First token (prefill)
            prefill_results = [x[:1] for x in non_greedy_results]
            assert_varied(prefill_results, 2, "With top_k=10, first tokens should vary in batch.")

        # Between batches:
        greedy_results_1 = results_1[:num_greedy]
        greedy_results_2 = results_2[:num_greedy]
        assert_deterministic(greedy_results_1+greedy_results_2, "Greedy requests should produce the same output when re-ran.")

        # Overall
        non_greedy_results_1 = results_1[num_greedy:]
        non_greedy_results_2 = results_2[num_greedy:]
        assert_pairwise_varied(non_greedy_results_1, non_greedy_results_2, len(non_greedy_results_1)//2, "Non-greedy requests should produce different outputs when re-ran.")

        # First token (prefill)
        prefill_results_1 = [x[:1] for x in non_greedy_results_1]
        prefill_results_2 = [x[:1] for x in non_greedy_results_2]
        assert_pairwise_varied(prefill_results_1, prefill_results_2, 2, "With top_k=10, first tokens should vary when re-ran.")


    def test_top1_is_greedy(
        self, tt_server, tt_model_name
    ):
        prompt = "Number: "
        greedy_config = RequestConfig(prompt=prompt, max_tokens=10, temperature=0)
        top1_config = RequestConfig(prompt=prompt, max_tokens=10, temperature=2.0, top_k=1)
        ...
        assert_deterministic(all_results, "top_k=1 requests should produce the same output across positions and runs.")
