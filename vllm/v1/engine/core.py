...
class DPEngineCoreProc(EngineCoreProc):
    ...
    def _init_dp_group(self, parallel_config: ParallelConfig) -> None:
        ...
        if self.requires_gather:
            ...
            # Build per-host DP subgroups ("segments") so device ranks only gather
            # and scatter within their local host segment.
            num_hosts = len(self.dp_device_ranks)
            assert num_hosts > 0, "No DP device ranks found"
            assert dp_size % num_hosts == 0, (
                f"DP size ({dp_size}) must be divisible by number of hosts "
                f"({num_hosts})"
            )
            self.dp_segment_size = dp_size // num_hosts
            # DP ranks are contiguous per host segment.
            self.dp_segment_start = (
                self.dp_rank // self.dp_segment_size
            ) * self.dp_segment_size
            self.dp_segment_group = None
            for h in range(num_hosts):
                ranks = list(
                    range(h * self.dp_segment_size, (h + 1) * self.dp_segment_size)
                )
                g = dist.new_group(ranks=ranks)
                if self.dp_segment_start == h * self.dp_segment_size:
                    self.dp_segment_group = g
            assert self.dp_segment_group is not None, (
                "Failed to create DP segment group"
            )
        else:
    ...
    def _execute_model_dp_gather(self, scheduler_output: SchedulerOutput):
        parallel_config = self.vllm_config.parallel_config
        assert (
            hasattr(self, "dp_segment_group") and self.dp_segment_group is not None
        ), "dp_segment_group must be initialized before DP gather execution"
        group = self.dp_segment_group
        rank = self.dp_rank
        local_rank = parallel_config.data_parallel_rank_local
        world = parallel_config.data_parallel_size
        assert hasattr(self, "dp_segment_size") and self.dp_segment_size is not None, (
            "dp_segment_size must be initialized before DP gather execution"
        )
        assert (
            hasattr(self, "dp_segment_start") and self.dp_segment_start is not None
        ), "dp_segment_start must be initialized before DP gather execution"
        segment_size = int(self.dp_segment_size)
        segment_root = int(self.dp_segment_start)
        ...
        if is_decode:
            ...
            # Only segment root needs buffers for the gather operation.
            # Pre-allocate stacked tensors and create list views for gather.
            stacked_int = None
            stacked_float = None
            gather_list_int = None
            gather_list_float = None
            if rank == segment_root:
                stacked_int = torch.empty(
                    (segment_size, *int_local.shape), dtype=int_local.dtype
                )
                stacked_float = torch.empty(
                    (segment_size, *float_local.shape), dtype=float_local.dtype
                )
                gather_list_int = [stacked_int[i] for i in range(segment_size)]
                gather_list_float = [stacked_float[i] for i in range(segment_size)]

            # Gather to per-host segment root (device rank for this host).
            dist.gather(int_local, gather_list_int, dst=segment_root, group=group)
            dist.gather(float_local, gather_list_float, dst=segment_root, group=group)
            ...
            if any_penalties_inputs and (not all_sample_device or any_reset_batch):
                # Gather token dicts (containing tensors) to segment root
                if rank == segment_root:
                    gathered_tokens_inputs = [None for _ in range(segment_size)]
            ...
            if not all_sample_device:
                if rank == segment_root:
                    gathered_host_only_sample_params = [
                        None for _ in range(segment_size)
                    ]
            ...
            if rank == segment_root:
                gathered_inputs = [None for _ in range(segment_size)]  # type: ignore
            ...
            # Gather to per-host segment root (device rank for this host).
            dist.gather_object(
                local_input, gathered_inputs, dst=segment_root, group=group
            )
        ...
        logprobs_per_dp: list = [None] * segment_size
        if local_rank == 0 and (
            is_decode
            or (
            ...
            send_tensor = torch.zeros((segment_size, B, 1), dtype=torch.int32)

        # Segment root scatters results to ranks on the same host.
        my_ids = torch.empty((B, 1), dtype=torch.int32)
        scatter_list = None
        if rank == segment_root:
            # Prepare scatter list: split send_tensor along first dimension
            scatter_list = [send_tensor[i] for i in range(segment_size)]
        dist.scatter(my_ids, scatter_list, src=segment_root, group=group)
        ...
        if any_needs_logprobs:
            ...
            logprobs_scatter_list = logprobs_per_dp if rank == segment_root else None
            dist.scatter_object_list(
                my_logprobs, logprobs_scatter_list, src=segment_root, group=group
            )
